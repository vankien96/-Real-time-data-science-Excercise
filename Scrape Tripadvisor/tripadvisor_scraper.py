...
def parse(locality,checkin_date,checkout_date,sort):
    ...
    for hotel in hotel_lists:
        ...
        url = 'http://www.tripadvisor.com'+raw_hotel_link[0] if raw_hotel_link else  ''
        ...
        price_per_night = ''.join(raw_hotel_price_per_night[0]).replace('\n','') if len(raw_hotel_price_per_night) != 0 else ''
...
if __name__ == '__main__':
    ...
    if today<datetime.strptime(checkIn,"%Y/%m/%d") and datetime.strptime(checkIn,"%Y/%m/%d")<datetime.strptime(checkOut,"%Y/%m/%d"):
        ...
        with open('tripadvisor_data.csv','wb') as csvfile:
            fieldnames = ['hotel_name', 'url', 'locality', 'reviews', 'tripadvisor_rating', 'checkOut', 'checkIn', 'hotel_features', 'price_per_night', 'no_of_deals', 'booking_provider']
